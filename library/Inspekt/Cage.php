...
{
	/**
	 * The raw source data. Although tempting, NEVER EVER
	...
	protected function _parseAndApplyAutoFilters($conf_file, $conf_section)
	{
		if (isset($conf_file))
		{
			...
			if (isset($conf_section) && isset($conf[$conf_section]))
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isAlnum($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isAlpha($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isBetween($this->_getValue($key), $min, $max, $inc))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isCcnum($this->_getValue($key), $type))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isDate($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isDigits($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isEmail($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isFloat($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isGreaterThan($this->_getValue($key), $min))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isHex($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isHostname($this->_getValue($key), $allow))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isInt($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isIp($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isLessThan($this->_getValue($key), $max))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isOneOf($this->_getValue($key), $allowed))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isPhone($this->_getValue($key), $country))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isRegex($this->_getValue($key), $pattern))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isUri($this->_getValue($key)))
			return false;
	...
	{
		if (!$this->keyExists($key) || !Inspekt::isZip($this->_getValue($key)))
			return false;
	...
	protected function _keyExistsRecursive($keys, $data_array)
	{
		...
		if (array_key_exists($thiskey, $data_array))
		{
			...
			if ($data_array[$thiskey] instanceof ArrayObject)
			{
				unset($keys[key($keys)]);
